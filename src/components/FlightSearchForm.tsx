...
const ARGENTINE_AIRPORTS = [
  { code: "AEP", name: "Buenos Aires - Aeroparque (AEP)" },
  { code: "EZE", name: "Buenos Aires - Ezeiza (EZE)" },
  { code: "COR", name: "Córdoba (COR)" },
  { code: "MDZ", name: "Mendoza (MDZ)" },
  { code: "BRC", name: "Bariloche (BRC)" },
  { code: "IGR", name: "Puerto Iguazú (IGR)" },
  { code: "SLA", name: "Salta (SLA)" },
  { code: "USH", name: "Ushuaia (USH)" },
  { code: "ROS", name: "Rosario (ROS)" },
  { code: "NQN", name: "Neuquén (NQN)" },
];
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <Select onValueChange={field.onChange} defaultValue={field.value} value={field.value}>
                        <FormControl>
                          <SelectTrigger className={cn("relative pl-10 text-left", !field.value && "text-muted-foreground")}>
                            <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                            <SelectValue placeholder="¿Desde dónde?" />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent>
                          {ARGENTINE_AIRPORTS.map((airport) => (
                    ...
                    <FormItem>
                      ...
                      <Select onValueChange={field.onChange} defaultValue={field.value} value={field.value}>
                        <FormControl>
                          <SelectTrigger className={cn("relative pl-10 text-left", !field.value && "text-muted-foreground")}>
                            <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                            <SelectValue placeholder="¿Adónde quieres ir?" />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent>
                          {ARGENTINE_AIRPORTS.map((airport) => (
